...
from api.endpoints.file.download_url import router as download_url_router
...
ROUTERS = (init_router, preview_router, download_router, download_url_router)
...
@pytest.mark.full_integration
class TestApiDownloadFileUrl:
    """Wariant JSON /files/download-url/{id} (bez 302) - patrz api/endpoints/file/download_url.py
    dla uzasadnienia (fetch() z Authorization podazajacy za cross-origin redirectem
    na S3 gubil CORS w realnej przegladarce)."""

    def test_download_url01_returns_working_attachment_presigned_url(self, db_session):
        client = make_client(db_session, *ROUTERS)
        user = create_test_user(db_session)
        image_bytes = IMAGE_PATH.read_bytes()
        s3_client = get_s3_client()
        ...
        with authorized_as("superadmin", user_id=str(user.id)) as headers:
            file_record = _upload_completed_file(
                client, headers, db_session, image_bytes, name="download_url.png"
            )

            try:
                response = client.get(f"/files/download-url/{file_record.id}", headers=headers)
                assert response.status_code == 200, response.text
                data = response.json()["data"]
                assert data["expires_in_seconds"] == 60

                # klient (frontend) robi teraz czysty GET bez zadnych naglowkow
                downloaded = requests.get(data["url"], timeout=30)
                assert downloaded.status_code == 200
                assert downloaded.content == image_bytes
                assert downloaded.headers["Content-Disposition"].startswith("attachment")
    ...
    def test_download_url02_pending_file_returns_409(self, db_session):
        client = make_client(db_session, *ROUTERS)
        user = create_test_user(db_session)
        ...
        with authorized_as("superadmin", user_id=str(user.id)) as headers:
            init_response = client.post(
                "/files/init",
                json={
                    "name": "still_pending3.png",
                    "original_name": "still_pending3.png",
                    "size": 10,
                    "mime_type": "image/png",
                    "file_type": "photo",
                    "catalog": "tests/api_preview_download",
            ...
            response = client.get(f"/files/download-url/{file_id}", headers=headers)

        assert response.status_code == 409

    def test_download_url03_not_found_returns_404(self, db_session):
        client = make_client(db_session, *ROUTERS)

        with authorized_as("superadmin") as headers:
            response = client.get(f"/files/download-url/{uuid4()}", headers=headers)

        assert response.status_code == 404

    def test_download_url04_invalid_uuid_returns_400(self, db_session):
        client = make_client(db_session, *ROUTERS)

        with authorized_as("superadmin") as headers:
            response = client.get("/files/download-url/not-a-uuid", headers=headers)
