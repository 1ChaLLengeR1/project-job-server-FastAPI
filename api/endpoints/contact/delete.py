...
from api.routers import DELETE_CONTACT_MESSAGE
...
from core.handler.contact.delete import handler_delete_contact_message
...
@router.delete(
    ...
    summary="[Superadmin] Usuń wiadomość kontaktową",
...
def api_superadmin_delete_contact_message(
    request: Request,
    message_id: str,
    ...
    try:
        if not is_valid_uuid(message_id):
            return invalid_uuid_response("Message_id", "api_superadmin_delete_contact_message")

        data, error, success = handler_delete_contact_message(user_data["id"], message_id, db_session=db)
        if not success:
        ...
        error = ApiErrorData(
            message=str(e),
            type_module="api_superadmin_delete_contact_message",
