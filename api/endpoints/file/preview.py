...
from api.routers import PREVIEW_FILE
...
from core.handler.file.preview import handler_preview_file
...
@router.get(
    PREVIEW_FILE,
    summary="[Superadmin] Krótkoterminowy presigned URL do podglądu pliku (inline)",
...
def api_superadmin_preview_file(
    request: Request,
    file_id: str,
    ...
    try:
        if not is_valid_uuid(file_id):
            return invalid_uuid_response("File_id", "api_superadmin_preview_file")

        data, error, success = handler_preview_file(user_data["id"], file_id, db_session=db)
        if not success:
        ...
        error = ApiErrorData(
            message=str(e),
            type_module="api_superadmin_preview_file",
