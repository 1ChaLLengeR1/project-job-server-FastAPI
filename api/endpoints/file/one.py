...
from api.routers import ONE_FILE
from api.schemas.file.response import FileWithChildrenResponseData
from api.validators import is_valid_uuid
from config.rate_limit import RATE_LIMIT_READ, auth_or_ip_key, limiter
from core.data.user import UserData
from core.handler.file.one import handler_one_file
...
@router.get(
    ONE_FILE,
    summary="[Superadmin] Pobierz plik (+ lista plików-dzieci)",
    response_model=ApiResponse[FileWithChildrenResponseData],
    responses={
...
def api_superadmin_one_file(
    request: Request,
    file_id: str,
    user_data: UserData = Depends(JWTBasicAuthenticationMiddleware(roles=["superadmin"])),
    db: Session = Depends(get_db),
) -> ApiResponse[FileWithChildrenResponseData] | JSONResponse:
    try:
        if not is_valid_uuid(file_id):
            return invalid_uuid_response("File_id", "api_superadmin_one_file")

        data, error, success = handler_one_file(user_data["id"], file_id, db_session=db)
        if not success:
        ...
        return ApiResponse(status="SUCCESS", status_code=200, data=FileWithChildrenResponseData(**asdict(data)))
    except Exception as e:
        error = ApiErrorData(
            message=str(e),
            type_module="api_superadmin_one_file",
