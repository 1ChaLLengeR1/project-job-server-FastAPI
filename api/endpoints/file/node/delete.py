...
from api.routers import DELETE_FILES_NODE
...
from core.handler.file.node.delete import handler_delete_files_node
...
@router.delete(
    ...
    responses={
        ...
        409: {
            ...
            "description": "Węzeł ma dzieci-węzły lub przypisane pliki (IntegrityError/RESTRICT)",
        },
...
def api_superadmin_delete_files_node(
    request: Request,
    node_id: str,
    ...
    try:
        if not is_valid_uuid(node_id):
            return invalid_uuid_response("Node_id", "api_superadmin_delete_files_node")

        data, error, success = handler_delete_files_node(user_data["id"], node_id, db_session=db)
        if not success:
        ...
        error = ApiErrorData(
            message=str(e),
            type_module="api_superadmin_delete_files_node",
