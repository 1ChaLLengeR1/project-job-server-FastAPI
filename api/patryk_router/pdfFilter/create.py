...
from fastapi import APIRouter, BackgroundTasks, Depends, File, Request, UploadFile, WebSocket
...
from core.helper.files import check_files_size, save_files_tmp
...
from core.services.websocekt.patryk_router.pdfFilter.websocket import websocket_endpoint
...
def create_pdf_filter(request: Request, background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    if not file.filename.lower().endswith(".xlsx"):
        return ResponseApiData(
            status="ERROR", data="Typ file is not .xlsx!", status_code=400, additional=None
        ).to_response()
    ...
    if check_files_size(files) > 0:
        ...
        return ResponseApiData(status="ERROR", data="File size is 0!", status_code=400, additional=None).to_response()
