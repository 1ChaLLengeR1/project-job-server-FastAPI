from config.app_config import AUTO_REMOVE_FILES, FILE
from core.data.response import ResponseData
from core.data.user import UserData
from core.services.patryk.pdfFilter.convert import convert_product_with_images
from core.services.patryk.pdfFilter.file import clear_catalog, clear_tmp_files, extract_images_from_xlsx, open_file_xlsx
from core.services.patryk.pdfFilter.pdf import generate_pdf
from core.services.patryk.pdfFilter.sum import sum_duplicat
    ...
    try:
        ...
        if not is_valid:
            return ResponseData(
                is_valid=is_valid,
                status="ERROR",
                data={"type": "extract_images_from_xlsx", "error": err},
                status_code=400,
                additional=None,
            )
        ...
        if not is_valid:
            return ResponseData(
                is_valid=is_valid,
                status="ERROR",
                data={"type": "open_file_xlsx", "error": err},
                status_code=400,
                additional=None,
            )
        ...
        if not is_valid:
            return ResponseData(
                is_valid=is_valid,
                status="ERROR",
                data={"type": "sum_duplicat", "error": err},
                status_code=400,
                additional=None,
            )
        ...
        if not is_valid:
            return ResponseData(
                is_valid=is_valid,
                status="ERROR",
                data={"type": "convert_product_with_images", "error": err},
                status_code=400,
                additional=None,
            )
        ...
        if not is_valid:
            return ResponseData(
                is_valid=is_valid,
                status="ERROR",
                data={"type": "generate_pdf", "error": err_or_path},
                status_code=400,
                additional=None,
            )
        ...
        return ResponseData(is_valid=True, status="SUCCESS", data=str(err_or_path), status_code=200, additional=None)
