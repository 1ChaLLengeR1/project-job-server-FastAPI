...
def handler_delete_contact_message(
    user_id: str, message_id: str, db_session: Session | None = None
) -> tuple[ContactMessageResponse | None, ApiErrorData | None, bool]:
    try:
        result, err, ok = delete_contact_message_psql(message_id, db_session=db_session)
        if not ok:
            return None, err, False

        create_logs_psql(user_id, "contact:delete_message", db_session=db_session)
        ...
        return (
            ...
            ApiErrorData(
                message=str(e),
                type_module="handler_delete_contact_message",
