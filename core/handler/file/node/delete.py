...
def handler_delete_files_node(
    user_id: str, node_id: str, db_session: Session | None = None
) -> tuple[FilesNodeResponse | None, ApiErrorData | None, bool]:
    try:
        result, err, ok = delete_files_node_psql(node_id, db_session=db_session)
        if not ok:
            return None, err, False

        create_logs_psql(user_id, "files:delete_node", db_session=db_session)
        return result, None, True
    except Exception as e:
        return None, ApiErrorData(
            message=str(e),
            type_module="handler_delete_files_node",
