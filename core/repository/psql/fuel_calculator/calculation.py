...
def fuel_calculation_psql(payload: FuelData) -> ResponseData:
    try:
        ...
        if len(errors) > 0:
            return ResponseData(is_valid=False, status="ERROR", data=errors, status_code=400, additional=None)

        price = ((payload["combustion"] / 100) * payload["fuel"] * payload["way"]) + payload["remaining_values"]
        pattern = (
            f"({payload['combustion']} / 100) * {payload['fuel']} * {payload['way']} + {payload['remaining_values']}"
        )

        data = {"price": price, "pattern": pattern}
