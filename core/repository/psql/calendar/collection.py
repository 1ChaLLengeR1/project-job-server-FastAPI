    ...
    try:
        ...
        if not row_collection:
            return create_error_response(
                message=f"For this year: {year} and month: {month} is not available.", status_code=400
            )
        ...
        db.rollback()
        return create_error_response(message=f"collection_calendar_psql error: {e}.", status_code=417)
    finally:
        db.close()
