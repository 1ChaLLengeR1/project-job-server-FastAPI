    ...
    try:
        ...
        if not last_work_condition:
            return create_error_response(
                message="Create first work condition, because database is empty.", status_code=400
            )
        ...
        if existing_count > 0:
            return create_error_response(message=f"For this year: {year} calendar existing.", status_code=409)
        ...
        try:
            ...
            db.rollback()
            return create_error_response(message=f"create_generator_calendar_psql IntegrityError: {e}", status_code=417)
        ...
        db.rollback()
        return create_error_response(message=f"create_generator_calendar_psql error: {e}.", status_code=417)
    finally:
        db.close()
