    ...
    try:
        ...
        for item in payload["array_object"]:
            new_outstanding_money = OutStandingMoney(
                amount=item["amount"], name=item["name"], date=datetime.now(), id_name=new_uuid4
            )
        ...
        data = {
            "names_overdue": {"id": str(new_uuid4), "name": payload["name"]},
...
def add_item_psql(user_data: UserData, payload: AddItemParams) -> ResponseData:
    ...
    try:
        ...
        new_item = OutStandingMoney(
            amount=payload["amount"], name=payload["name"], date=datetime.now(), id_name=id_name_uuid
        )
        db.add(new_item)
        db.commit()
