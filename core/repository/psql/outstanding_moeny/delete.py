...
def delete_list_psql(user_data: UserData, id: str) -> ResponseData:
    ...
    try:
        ...
        data = {
            "name_overdue": {"id": str(item_names_overdue.id), "name": item_names_overdue.name},
            "outstanding_money": [
                {
                    "id": str(item.id),
                    "amount": item.amount,
...
def delete_item_psql(user_data: UserData, id: str) -> ResponseData:
    ...
    try:
        ...
        return ResponseData(is_valid=True, status="SUCCESS", data=deleted_item_data, status_code=200, additional=None)
