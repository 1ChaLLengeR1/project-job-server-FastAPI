...
def collection_list_psql() -> ResponseData:
    ...
    try:
        ...
        for item in item_names_overdue:
            object_item = {"id_name": str(item.id), "name_overdue": item.name, "array_items": [], "full_price": 0}

            item_outstanding_money = (
                db.query(OutStandingMoney)
                .filter(OutStandingMoney.id_name == str(item.id))
        ...
        return ResponseData(is_valid=True, status="SUCCESS", data=list_overdue, status_code=200, additional=None)
