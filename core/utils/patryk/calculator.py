...
def shop_cost(user_id: str or int, referrer: str, db):
    keys_id_price = (
        db.query(KeysCalculatorPatryk)
        .with_entities(
            KeysCalculatorPatryk.inpost_parcel_locker,
            ...
            KeysCalculatorPatryk.without_smart,
        )
        .filter(KeysCalculatorPatryk.id == user_id)
        .first()
    )
...
def calculations_calculator(
    package_price: float,
    vat: float,
    cena_netto_sprzedarz: float,
    cena_netto_zakup: float,
    prowizja_wyroznienie_netto: float,
    dochodowka: float,
    roznica_vat_sprzedarz: float,
    roznica_vat_zakupu: float,
    roznica_vat_prowizji_wyroznienie: float,
):
    ...
    return {"brutto": round(brutto, 2), "na_czysto": round(na_czysto, 2), "zysk_procentowy": round(zysk_procentowy, 2)}
